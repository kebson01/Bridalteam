<?php 
	get_header(); 
	global $_wedfolio;
	$term = get_query_var('vendor-category');
	$term = get_term_by( 'slug', $term, 'vendor-category' );
?>

<div id="container" class="vendor-category">
	<article class="item welcome-cats">
		<h2><?php echo $term->name; ?></h2>
		<a href="<?php echo home_url('/vendor/'); ?>" class="view-all">view all categories</a>
		<?php $wedcats = get_categories(array('taxonomy' => 'vendor-category', 'hide_empty' => false)); ?>
		<ul class="wedcats cats">
			<?php $i=1; foreach ($wedcats as $wc) : ?>
				<li class="cat-item gradient<?php echo $i; ?>"><a href="<?php echo get_term_link( $wc, 'vendor-category' ); ?>"><?php echo $wc->name; ?></a></li>
			<?php $i++; endforeach; ?>
		</ul>		
	</article>

	<article class="item welcome-cats welcome-locs">
		<h2 class="location">Filter<span>by location</span></h2>
		<div class="dropdown">
		<?php wp_dropdown_categories(array('taxonomy' => 'vendor-location')); ?>
		</div>
	</article>
...
$(window).load(function(){
	...
	$('.dropdown select').dropkick({
	  change: function (value, label) {
	   var url = 'http://' + $(location).attr('host') + $(location).attr('pathname');
	   $(location).attr('href', url+'?vc='+value );
